...
@gluon.jit
def _router_linear(X, W, L, M: gl.constexpr, H: gl.constexpr, SX: gl.constexpr, BM: gl.constexpr, BN: gl.constexpr, BK: gl.constexpr, Counts, pid_m, pid_n, WARPS: gl.constexpr=4, INIT_SHARDS: gl.constexpr=8, SPLITS: gl.constexpr=1, split=0):
    init_shard = pid_m * (256 // BN) + pid_n
    if (init_shard < INIT_SHARDS) & (split == 0):
...
@gluon.jit
def _router_and_quantize(X, W, L, Q, QS, Counts, M: gl.constexpr, H: gl.constexpr, SX: gl.constexpr, BM: gl.constexpr, BN: gl.constexpr, BK: gl.constexpr, GROUPS: gl.constexpr, WARPS: gl.constexpr, SHARDS: gl.constexpr, SPLITS: gl.constexpr):
    ROUTER_ROWS: gl.constexpr = triton.cdiv(M, BM)
    ROUTER_CTAS: gl.constexpr = ROUTER_ROWS * (256 // BN) * SPLITS
    pid = gl.program_id(0)
    if pid < ROUTER_CTAS:
        _router_linear(X, W, L, M, H, SX, BM, BN, BK, Counts, pid % ROUTER_ROWS, pid // ROUTER_ROWS % (256 // BN), WARPS, SHARDS, SPLITS, pid // (ROUTER_ROWS * (256 // BN)))
    else:
...
@gluon.jit
def _load_packed_weight(W, S, expert, column, base, N: gl.constexpr, K: gl.constexpr, BN: gl.constexpr, BK: gl.constexpr, UP: gl.constexpr, WEIGHT_CACHE: gl.constexpr='', WARPS: gl.constexpr=4, DIRECT_WEIGHT: gl.constexpr=False):
    W = W + expert * (N * K // 2)
    S = S + expert * (N * (triton.cdiv(K // 32, 8) * 8))
    packed: gl.constexpr = gl.BlockedLayout([1, 4], [16, 4] if DIRECT_WEIGHT else [32, 2], [WARPS, 1], [0, 1] if DIRECT_WEIGHT else [1, 0])
...
@gluon.jit
def _scaled_tile(X, XS, W, WS, Sorted, Q, QS, Parts, Y, job, column, expert, live, expert_tile, M: gl.constexpr, N: gl.constexpr, K: gl.constexpr, BN: gl.constexpr, BK: gl.constexpr, UP: gl.constexpr, WARPS: gl.constexpr, DIRECT_WEIGHT: gl.constexpr, WEIGHT_CACHE: gl.constexpr):
    TM: gl.constexpr = 16
...
@gluon.jit
def _scaled_experts(X, XS, W, WS, Sorted, Counts, Jobs, Q, QS, Parts, Y, M: gl.constexpr, N: gl.constexpr, K: gl.constexpr, BN: gl.constexpr, BK: gl.constexpr, GROUP: gl.constexpr, UP: gl.constexpr, WARPS: gl.constexpr, DIRECT_WEIGHT: gl.constexpr=False, WEIGHT_CACHE: gl.constexpr='.cg'):
    pid = gl.program_id(0)
    ...
    if job < routed_jobs + gl.cdiv(M, 16):
        ...
        _scaled_tile(X, XS, W, WS, Sorted, Q, QS, Parts, Y, job, column, expert, live, expert_tile, M, N, K, BN, BK, UP, WARPS, DIRECT_WEIGHT, WEIGHT_CACHE)
...
def fused_moe(x, router, correction_bias, w13, w13_scale, w2, w2_scale):
    ...
    router_splits = 4 if h % 2048 == 0 else 1
    router_m, router_n = (16, 16)
    router_k = 512 if h % 512 == 0 else 256
    if m <= 32:
        up_n, up_k, up_warps = (128, router_k, 2)
        down_n, down_warps = (256, 4)
        up_group = 4
    else:
        up_n, up_k, up_warps = (64, 256, 2)
        down_n, down_warps = (128, 2)
        up_group = 1
    up_direct = m <= 32
    down_cache = '.cg' if m <= 32 else ''
    down_group = 1
    quant_groups = 16
    active_expert_bound = min(256, m * 8)
    routed_job_bound = active_expert_bound + (m * 8 - active_expert_bound) // 16
    jobs = triton.cdiv(routed_job_bound + triton.cdiv(m, 16), 4) * 4
    ...
    _router_and_quantize[router_ctas + quant_ctas,](x, router, logits, xq, xs, counts, m, h, x.stride(0), router_m, router_n, router_k, quant_groups, 1, 1, router_splits, num_warps=1, enable_fp_fusion=False)
    ...
    _scaled_experts[jobs * (2 * intermediate // up_n),](xq, xs, w13, w13_scale, sorted_routes, counts, job_info, aq, aqs, parts, out, m, 2 * intermediate, h, up_n, up_k, up_group, True, up_warps, up_direct, '', num_warps=up_warps, enable_fp_fusion=False)
    _scaled_experts[jobs * (h // down_n),](aq, aqs, w2, w2_scale, sorted_routes, counts, job_info, aq, aqs, parts, out, m, h, intermediate, down_n, 256, down_group, False, down_warps, True, down_cache, num_warps=down_warps)
