...
def _expert_projection(X, XS, W, WS, Ids, Groups, Y, N: gl.constexpr, K: gl.constexpr, M: gl.constexpr, UP: gl.constexpr, SPLITS: gl.constexpr, BN: gl.constexpr, BK: gl.constexpr, SHARED_ONLY: gl.constexpr=False, STAGGER: gl.constexpr=False):
    ...
    if owner:
        ...
        for block in range(K // SPLITS // BK):
            ...
            scale_offsets = _scale_offset(0, sn[:, None], start + sbg[None, :] * 32, N, K)
            if BK == 256:
                scale_words = gl.load(scale_base.to(gl.pointer_type(gl.uint32)) + scale_offsets // 4)
                ws = (scale_words >> 8 * (scale_offsets % 4)).to(gl.uint8)
            else:
                ws = gl.load(scale_base + scale_offsets)
...
@gluon.jit
def _activation_tile(GU, Q, QS, route, tile, I: gl.constexpr, SPLITS: gl.constexpr, WARPS: gl.constexpr, SHARED: gl.constexpr=None):
    ...
    shared = route % 9 == 8 if SHARED is None else SHARED
...
def _up_and_shared_activation(X, XS, W, WS, Ids, Groups, GU, AQ, AQS, H: gl.constexpr, I: gl.constexpr, M: gl.constexpr, SPLITS: gl.constexpr, BN: gl.constexpr, BK: gl.constexpr):
    ...
    if program < M * (I // 64):
        ...
        tile = program % (I // 64)
        _activation_tile(GU, AQ, AQS, route, tile, I, SPLITS, 1, SHARED=True)
    else:
...
def _activation_and_shared_down(GU, AQ, AQS, W, WS, Ids, Groups, Shared, H: gl.constexpr, I: gl.constexpr, M: gl.constexpr, SPLITS: gl.constexpr):
    ...
    if program < H // 64:
        ...
        route = job // 8 * 9 + job % 8
        tile = work % (I // 64)
        _activation_tile(GU, AQ, AQS, route, tile, I, SPLITS, 1, SHARED=False)

@gluon.jit
def _routed_down_batch(X, XS, W, WS, Ids, token, tile, N: gl.constexpr, K: gl.constexpr, BN: gl.constexpr):
    gl.static_assert(BN == 32 and K == 256)
    mma: gl.constexpr = gl.amd.AMDMFMALayout(version=4, instr_shape=[16, 16, 128], transposed=False, warps_per_cta=[1, 8], tiles_per_warp=[1, 2])
    al: gl.constexpr = gl.BlockedLayout([1, 16], [16, 4], [1, 8], [1, 0])
    pl: gl.constexpr = gl.DistributedLinearLayout(reg_bases=[[0, 1], [0, 2], [16, 0], [0, 16]], lane_bases=[[1, 0], [2, 0], [4, 0], [8, 0], [0, 4], [0, 8]], warp_bases=[[32, 0], [64, 0], [128, 0]], block_bases=[], shape=[8 * BN, K // 8])
    dot_a: gl.constexpr = gl.DotOperandLayout(0, mma, 16)
    dot_b: gl.constexpr = gl.DotOperandLayout(1, mma, 16)
    ...
    scale_offsets = _scale_offset(se[:, None], tile * BN + sn[:, None] % BN, sbg[None, :] * 32, N, K)
    scale_words = gl.load(WS.to(gl.pointer_type(gl.uint32)) + scale_offsets // 4)
    ws = (scale_words >> 8 * (scale_offsets % 4)).to(gl.uint8)
...
def _down_combine(X, XS, W, Scales, Ids, Weights, Shared, Y, H: gl.constexpr, I: gl.constexpr, BN: gl.constexpr):
    ...
    layout: gl.constexpr = gl.DistributedLinearLayout(reg_bases=[[1, 0], [2, 0], [4, 0], [0, 16]], lane_bases=[[0, 1], [0, 2], [0, 4], [0, 8], [0, 0], [0, 0]], warp_bases=[[0, 0]] * 3, block_bases=[], shape=[8, BN])
...
def fused_moe(x, router, correction_bias, w13, w13_scale, w2, w2_scale) -> torch.Tensor:
    ...
    routes = m * 9
    stagger = m <= 8
    splits = 24 if stagger else 6
    up_width = 64 if stagger else 16
    up_block = 256 if stagger else 128
    ...
    if stagger:
        ...
        _down_combine[h // 32, m](aq, aqs, w2, w2_scale, ids, weights, parts, out, h, intermediate, 32, num_warps=8, enable_fp_fusion=False)
    else:
