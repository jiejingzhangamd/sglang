...
@gluon.jit
def _routed_down_batch(X, W, Scales, Ids, token, tile, N: gl.constexpr, K: gl.constexpr, BN: gl.constexpr, TWARPS: gl.constexpr):
    BK: gl.constexpr = 128
    CN: gl.constexpr = 8 * BN
    mma: gl.constexpr = gl.amd.AMDMFMALayout(version=4, instr_shape=[16, 16, 128], transposed=False, warps_per_cta=[1, TWARPS])
    al: gl.constexpr = gl.BlockedLayout([1, 16], [16, 4], [1, TWARPS], [1, 0])
    bl: gl.constexpr = gl.BlockedLayout([1, 4], [16, 4], [TWARPS, 1], [0, 1])
    ad: gl.constexpr = gl.DotOperandLayout(0, mma, 16)
    bd: gl.constexpr = gl.DotOperandLayout(1, mma, 16)
    asl: gl.constexpr = gl.amd.cdna4.get_mfma_scale_layout(ad, [16, BK // 32])
...
@gluon.jit
def _down_combine(X, W, Scales, Ids, Weights, Shared, Y, H: gl.constexpr, I: gl.constexpr, BN: gl.constexpr, TWARPS: gl.constexpr):
    tile = gl.program_id(0)
    token = gl.program_id(1)
    parts = _routed_down_batch(X, W, Scales, Ids, token, tile, H, I, BN, TWARPS)
    layout: gl.constexpr = gl.DistributedLinearLayout(reg_bases=[[1, 0], [2, 0], [4, 0]] + ([[0, 16]] if BN == 32 else []), lane_bases=[[0, 1], [0, 2], [0, 4], [0, 8], [0, 0], [0, 0]], warp_bases=[[0, 0]] * (1 if TWARPS == 2 else 2 if TWARPS == 4 else 3), block_bases=[], shape=[8, BN])
...
def fused_moe(x, router, correction_bias, w13, w13_scale, w2, w2_scale) -> torch.Tensor:
    ...
    grouped = m >= 2
    stagger = m <= 8
    splits = 16 if m == 1 else 8 if 3 <= m <= 8 else 12
    router_splits = 12
    router_block = 512
    quant_vector = 4 if m > 8 else 2 if 3 <= m <= 8 else 1
    ...
    if stagger:
        ...
        tail_warps = 2 if m == 2 else 4 if 3 <= m <= 8 else 8
        _down_combine[h // tail_width, m](aq, w2, w2_scale, ids, weights, down_output, out, h, intermediate, tail_width, tail_warps, num_warps=tail_warps, enable_fp_fusion=False)
    else:
        jobs = m * 8 + 1
        activation_warps = 4
