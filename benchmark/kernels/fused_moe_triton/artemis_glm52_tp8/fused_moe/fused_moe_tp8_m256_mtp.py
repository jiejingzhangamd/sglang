...
@gluon.jit
def _router_and_quantize(X, W, L, Q, QS, Counts, M: gl.constexpr, H: gl.constexpr, SX: gl.constexpr, BM: gl.constexpr, BN: gl.constexpr, BK: gl.constexpr, GROUPS: gl.constexpr, WARPS: gl.constexpr, SPLITS: gl.constexpr, Jobs, JOBS: gl.constexpr, PAD: gl.constexpr, TM: gl.constexpr):
    ...
    if pid == 0:
        ...
        gl.store(Counts + e, 0)
        j = gl.arange(0, _artifact_next_power_of_2(JOBS), jl)
        descriptor = gl.where(j < triton.cdiv(M, TM), 256 * PAD + j * TM, -1)
        gl.store(Jobs + j, descriptor, j < JOBS)
        gl.store(Counts + 256, triton.cdiv(M, TM))
    if pid < ROUTER_CTAS:
...
@gluon.jit
def _select_routes(L, Bias, Weights, Counts, Sorted, Jobs, M: gl.constexpr, SPLITS: gl.constexpr, PAD: gl.constexpr, TM: gl.constexpr):
    ...
    first = (e < 8) & (ticket % TM == 0)
    publish = first.to(gl.int32)
    job_count = gl.sum(publish, 0)
    job_base = gl.atomic_add(Counts + 256, job_count, job_count > 0, sem='relaxed')
    job = job_base + gl.associative_scan(publish, 0, _add) - publish
    arena = selected_id * PAD + ticket
    gl.store(Jobs + job, arena, first)
...
@gluon.jit
def _load_packed_weight(W, S, expert, column, base, N: gl.constexpr, K: gl.constexpr, BN: gl.constexpr, BK: gl.constexpr, UP: gl.constexpr, CACHE: gl.constexpr='', MDIM: gl.constexpr=16):
    WARPS: gl.constexpr = gl.num_warps()
    W = W + expert * (N * K // 2)
    S = S + expert * (N * (triton.cdiv(K // 32, 8) * 8))
    packed: gl.constexpr = gl.BlockedLayout([1, 4], [MDIM, 64 // MDIM], [WARPS, 1], [0, 1])
...
@gluon.jit
def _scaled_tile(X, XS, W, WS, Sorted, Q, QS, Parts, Y, arena_row, scratch_row, column, expert, live, M: gl.constexpr, N: gl.constexpr, K: gl.constexpr, PAD: gl.constexpr, BN: gl.constexpr, BK: gl.constexpr, UP: gl.constexpr, TM: gl.constexpr):
    WARPS: gl.constexpr = gl.num_warps()
    MDIM: gl.constexpr = 32 if TM == 32 and (not UP) else 16
    mma: gl.constexpr = gl.amd.AMDMFMALayout(version=4, instr_shape=[MDIM, MDIM, 128 if MDIM == 16 else 64], transposed=True, warps_per_cta=[1, WARPS])
    ...
    for base in range(K // BK):
        ...
        words, scales = _load_packed_weight(W, WS, expert, column, base, N, K, BN, BK, UP, '.cg' if UP else '', MDIM)
...
@gluon.jit
def _expert_tiles(X, XS, W, WS, Sorted, Jobs, Counts, Q, QS, Parts, Y, M: gl.constexpr, N: gl.constexpr, K: gl.constexpr, PAD: gl.constexpr, GROUP: gl.constexpr, UP: gl.constexpr, BN: gl.constexpr, BK: gl.constexpr, TM: gl.constexpr):
    ...
    if descriptor >= 0:
        ...
        live = gl.minimum(TM, count - start)
        scratch_row = tile * TM
        _scaled_tile(X, XS, W, WS, Sorted, Q, QS, Parts, Y, arena, scratch_row, column, expert, live, M, N, K, PAD, BN, BK, UP, TM)

@gluon.jit
def _reduce_parts(P, Y, Weights, M: gl.constexpr, H: gl.constexpr):
    BLOCK: gl.constexpr = 256
...
def fused_moe(x, router, correction_bias, w13, w13_scale, w2, w2_scale):
    ...
    tile_m = 16 if m <= 128 else 32
    pad = triton.cdiv(m, tile_m) * tile_m
    active_bound = min(256, m * 8)
    job_bound = active_bound + (m * 8 - active_bound) // tile_m + triton.cdiv(m, tile_m)
    ...
    up_n = 64
    up_warps = 2
    up_k = 1024 if tile_m == 16 else 512
    router_splits = 8
    router_k = 256
    up_group = 1 if tile_m == 16 else 4
    down_group = 1
    down_n = 256 if tile_m == 16 else 128
    down_warps = 4 if tile_m == 16 else 2
    quant_groups = 32
    ...
    activation_rows = jobs_count * tile_m
    ...
    _router_and_quantize[router_ctas + quant_ctas,](x, router, logits, xq, xs, counts, m, h, x.stride(0), 16, 16, router_k, quant_groups, 1, router_splits, jobs, jobs_count, pad, tile_m, num_warps=1, enable_fp_fusion=False)
    _select_routes[m,](logits, correction_bias, weights, counts, sorted_routes, jobs, m, router_splits, pad, tile_m, num_warps=1, enable_fp_fusion=False)
    _expert_tiles[jobs_count * (2 * intermediate // up_n),](xq, xs, w13, w13_scale, sorted_routes, jobs, counts, aq, aqs, parts, out, m, 2 * intermediate, h, pad, up_group, True, up_n, up_k, tile_m, num_warps=up_warps, enable_fp_fusion=False)
    _expert_tiles[jobs_count * (h // down_n),](aq, aqs, w2, w2_scale, sorted_routes, jobs, counts, aq, aqs, parts, out, m, h, intermediate, pad, down_group, False, down_n, 256, tile_m, num_warps=down_warps)
    _reduce_parts[m, h // 256](parts, out, weights, m, h, num_warps=1, enable_fp_fusion=False)
    return out
